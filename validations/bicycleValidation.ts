...
export const getBicyclesQuerySchema = z.object({
    ...
    brands: z.string().optional(), // comma-separated list of brand ids
    colors: z.string().optional(), // comma-separated list of colors
    sellerId: z.string().optional(),
    minPrice: z.coerce.number().min(0).optional(),
    maxPrice: z.coerce.number().min(0).optional(),
    usageMin: z.coerce.number().int().min(0).optional(),
    usageMax: z.coerce.number().int().min(0).optional(),
    yearMin: z.coerce.number().int().min(1900).max(new Date().getFullYear()).optional(),
    yearMax: z.coerce.number().int().min(1900).max(new Date().getFullYear()).optional(),
    inspectionStatus: z.enum(['PENDING', 'REQUESTED', 'ASSIGNED', 'IN_PROGRESS', 'COMPLETED', 'REJECTED']).optional(),
